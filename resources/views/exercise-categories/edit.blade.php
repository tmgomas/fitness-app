    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-semibold">Edit Exercise Category</h2>
                <a href="{{ route('exercise-categories.index') }}" class="text-gray-500 hover:text-gray-700">
                    Back to List
                </a>
            </div>

            <form action="{{ route('exercise-categories.update', $exerciseCategory) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-6">
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $exerciseCategory->name) }}"
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#f84525] focus:ring focus:ring-[#f84525] focus:ring-opacity-50 @error('description') border-red-500 @enderror">{{ old('description', $exerciseCategory->description) }}</textarea>
                            @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="measurement_type" id="measurement_type"
                                value="{{ old('measurement_type', $exerciseCategory->measurement_type) }}"
