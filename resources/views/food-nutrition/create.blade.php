    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-semibold">Create Food Nutrition</h2>
                <a href="{{ route('food-nutrition.index') }}" class="text-gray-500 hover:text-gray-700">
                    Back to List
                </a>
            </div>

            <form action="{{ route('food-nutrition.store') }}" method="POST">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="food_id" id="food_id"
                            ...
                            @foreach($foodItems as $foodItem)
                                <option value="{{ $foodItem->id }}" {{ old('food_id') == $foodItem->id ? 'selected' : '' }}>
                                    {{ $foodItem->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="nutrition_id" id="nutrition_id"
                            ...
                            @foreach($nutritionTypes as $type)
                                <option value="{{ $type->id }}" {{ old('nutrition_id') == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="amount_per_100g" id="amount_per_100g" value="{{ old('amount_per_100g') }}"
