        ...
        @foreach($nutritionTypes as $type)
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 items-center">
                <div>
                    ...
                    <input type="hidden" name="nutrition[{{ $loop->index }}][nutrition_id]" value="{{ $type->nutrition_id }}">
                    @if($existingNutrition)
                        <input type="hidden" name="nutrition[{{ $loop->index }}][food_nutrition_id]" value="{{ $existingNutrition->food_nutrition_id }}">
                    @endif
                </div>
                <div>
                    <input type="number" step="0.01" 
                           name="nutrition[{{ $loop->index }}][amount_per_100g]"
                ...
                <div class="flex items-center">
                    <input type="hidden" 
                           name="nutrition[{{ $loop->index }}][measurement_unit]" 
                           value="{{ $type->unit }}">
                    <span class="text-sm text-gray-500">{{ $type->unit }}</span>
                    
                    @if($existingNutrition)
                        <span class="ml-2 text-xs text-gray-400">(Current: {{ $existingNutrition->amount_per_100g }} {{ $type->unit }})</span>
                    @endif
                </div>
            </div>
        @endforeach
